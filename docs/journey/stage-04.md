# Stage 04 · CRUD, updates, and recovery
...
??? note "File diff: tests/test_update.py"
    ```diff
...
??? note "File diff: src/minimongodb/oplog/replay.py"
    ```diff
...
??? note "File diff: src/minimongodb/update/operators.py"
    ```diff
